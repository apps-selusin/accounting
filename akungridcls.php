...
$akun_grid = NULL; // Initialize page object first

class cakun_grid extends cakun {
	...
	// Page object name
	var $PageObjName = 'akun_grid';

	// Grid form hidden field names
	var $FormName = 'fakungrid';
...
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["akun"];
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function Page_Main() {
		...
		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "subgrup") {
			global $subgrup;
			$rsmaster = $subgrup->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("subgruplist.php"); // Return to master page
			} else {
				$subgrup->LoadListRowValues($rsmaster);
				$subgrup->RowType = EW_ROWTYPE_MASTER; // Master row
				$subgrup->RenderListRow();
				$rsmaster->Close();
			}
		}
	...
	function EmptyRow() {
		global $objForm;
		if ($objForm->HasValue("x_group_id") && $objForm->HasValue("o_group_id") && $this->group_id->CurrentValue <> $this->group_id->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_subgrup_id") && $objForm->HasValue("o_subgrup_id") && $this->subgrup_id->CurrentValue <> $this->subgrup_id->OldValue)
			return FALSE;
		...
		if ($objForm->HasValue("x_nama") && $objForm->HasValue("o_nama") && $this->nama->CurrentValue <> $this->nama->OldValue)
			return FALSE;
		if ($objForm->HasValue("x_matauang_id") && $objForm->HasValue("o_matauang_id") && $this->matauang_id->CurrentValue <> $this->matauang_id->OldValue)
			return FALSE;
		return TRUE;
	}
	...
	function ResetCmd() {
		...
		if (substr($this->Command,0,5) == "reset") {
			...
			if ($this->Command == "resetall") {
				...
				$this->DbMasterFilter = "";
				$this->DbDetailFilter = "";
				$this->subgrup_id->setSessionValue("");
			}
	...
	function LoadFormValues() {
		...
		$this->group_id->setOldValue($objForm->GetValue("o_group_id"));
		if (!$this->subgrup_id->FldIsDetailKey) {
			$this->subgrup_id->setFormValue($objForm->GetValue("x_subgrup_id"));
		}
		$this->subgrup_id->setOldValue($objForm->GetValue("o_subgrup_id"));
		...
		$this->matauang_id->setOldValue($objForm->GetValue("o_matauang_id"));
	...
	function RenderRow() {
		...
		$this->matauang_id->ViewCustomAttributes = "";
			...
			if ($this->subgrup_id->getSessionValue() <> "") {
				$this->subgrup_id->CurrentValue = $this->subgrup_id->getSessionValue();
				$this->subgrup_id->OldValue = $this->subgrup_id->CurrentValue;
			if (strval($this->subgrup_id->CurrentValue) <> "") {
				$sFilterWrk = "`id`" . ew_SearchString("=", $this->subgrup_id->CurrentValue, EW_DATATYPE_NUMBER, "");
			$sSqlWrk = "SELECT `id`, `kode` AS `DispFld`, `nama` AS `Disp2Fld`, '' AS `Disp3Fld`, '' AS `Disp4Fld` FROM `subgrup`";
	...
	function AddRow($rsold = NULL) {
		...
		// Set up foreign key field value from Session
			if ($this->getCurrentMasterTable() == "subgrup") {
				$this->subgrup_id->CurrentValue = $this->subgrup_id->getSessionValue();
			}
		$conn = &$this->Connection();
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "subgrup") {
			$this->subgrup_id->Visible = FALSE;
			if ($GLOBALS["subgrup"]->EventCancelled) $this->EventCancelled = TRUE;
		}
