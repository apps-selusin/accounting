...
$jurnal_view = NULL; // Initialize page object first

class cjurnal_view extends cjurnal {
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "jurnallist.php"; // No matching record, return to list
					} else {
						$this->LoadRowValues($this->Recordset); // Load row values
					}
			}
		} else {
			$sReturnUrl = "jurnallist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupOtherOptions() {
		...
		// "detail_jurnald"
		$item = &$option->Add("detail_jurnald");
		$body = $Language->Phrase("ViewPageDetailLink") . $Language->TablePhrase("jurnald", "TblCaption");
		...
		$links = "";
		if ($GLOBALS["jurnald_grid"] && $GLOBALS["jurnald_grid"]->DetailView) {
		...
		if ($GLOBALS["jurnald_grid"] && $GLOBALS["jurnald_grid"]->DetailEdit) {
		...
		if ($GLOBALS["jurnald_grid"] && $GLOBALS["jurnald_grid"]->DetailAdd) {
		...
		if ($sDetailTblVar <> "") {
			...
			if (in_array("jurnald", $DetailTblVar)) {
				...
				if ($GLOBALS["jurnald_grid"]->DetailView) {
					$GLOBALS["jurnald_grid"]->CurrentMode = "view";
...
// Create page object
if (!isset($jurnal_view)) $jurnal_view = new cjurnal_view();

// Page init
$jurnal_view->Page_Init();

// Page main
$jurnal_view->Page_Main();
...
var CurrentForm = fjurnalview = new ew_Form("fjurnalview", "view");

// Form_CustomValidate event
fjurnalview.Form_CustomValidate = 
...
fjurnalview.ValidateRequired = true;
<?php } else { ?>
fjurnalview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fjurnalview.Lists["x_tipejurnal_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_nama","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tipejurnal"};
fjurnalview.Lists["x_period_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_start","x_end","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"periode"};
...
<div class="ewToolbar">
<?php if (!$jurnal_view->IsModal) { ?>
<?php $Breadcrumb->Render(); ?>
<?php } ?>
<?php $jurnal_view->ExportOptions->Render("body") ?>
<?php
	foreach ($jurnal_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if (!$jurnal_view->IsModal) { ?>
...
?>
<form name="fjurnalview" id="fjurnalview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($jurnal_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $jurnal_view->Token ?>">
<?php } ?>
...
<?php if ($jurnal->id->Visible) { // id ?>
	<tr id="r_id">
		<td><span id="elh_jurnal_id"><?php echo $jurnal->id->FldCaption() ?></span></td>
		<td data-name="id"<?php echo $jurnal->id->CellAttributes() ?>>
<span id="el_jurnal_id">
<span<?php echo $jurnal->id->ViewAttributes() ?>>
<?php echo $jurnal->id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($jurnal->tipejurnal_id->Visible) { // tipejurnal_id ?>
	<tr id="r_tipejurnal_id">
		<td><span id="elh_jurnal_tipejurnal_id"><?php echo $jurnal->tipejurnal_id->FldCaption() ?></span></td>
		<td data-name="tipejurnal_id"<?php echo $jurnal->tipejurnal_id->CellAttributes() ?>>
<span id="el_jurnal_tipejurnal_id">
<span<?php echo $jurnal->tipejurnal_id->ViewAttributes() ?>>
<?php echo $jurnal->tipejurnal_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($jurnal->period_id->Visible) { // period_id ?>
	<tr id="r_period_id">
		<td><span id="elh_jurnal_period_id"><?php echo $jurnal->period_id->FldCaption() ?></span></td>
		<td data-name="period_id"<?php echo $jurnal->period_id->CellAttributes() ?>>
<span id="el_jurnal_period_id">
<span<?php echo $jurnal->period_id->ViewAttributes() ?>>
<?php echo $jurnal->period_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php echo $jurnal->createon->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($jurnal->keterangan->Visible) { // keterangan ?>
	<tr id="r_keterangan">
		<td><span id="elh_jurnal_keterangan"><?php echo $jurnal->keterangan->FldCaption() ?></span></td>
		<td data-name="keterangan"<?php echo $jurnal->keterangan->CellAttributes() ?>>
...
<?php if ($jurnal->person_id->Visible) { // person_id ?>
	<tr id="r_person_id">
		<td><span id="elh_jurnal_person_id"><?php echo $jurnal->person_id->FldCaption() ?></span></td>
		<td data-name="person_id"<?php echo $jurnal->person_id->CellAttributes() ?>>
<span id="el_jurnal_person_id">
<span<?php echo $jurnal->person_id->ViewAttributes() ?>>
<?php echo $jurnal->person_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if (!$jurnal_view->IsModal) { ?>
<?php if (!isset($jurnal_view->Pager)) $jurnal_view->Pager = new cPrevNextPager($jurnal_view->StartRec, $jurnal_view->DisplayRecs, $jurnal_view->TotalRecs) ?>
<?php if ($jurnal_view->Pager->RecordCount > 0 && $jurnal_view->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($jurnal_view->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $jurnal_view->PageUrl() ?>start=<?php echo $jurnal_view->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($jurnal_view->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $jurnal_view->PageUrl() ?>start=<?php echo $jurnal_view->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $jurnal_view->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($jurnal_view->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $jurnal_view->PageUrl() ?>start=<?php echo $jurnal_view->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($jurnal_view->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $jurnal_view->PageUrl() ?>start=<?php echo $jurnal_view->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $jurnal_view->Pager->PageCount ?></span>
</div>
<?php } ?>
<div class="clearfix"></div>
<?php } ?>
<?php
	if (in_array("jurnald", explode(",", $jurnal->getCurrentDetailTable())) && $jurnald->DetailView) {
?>
