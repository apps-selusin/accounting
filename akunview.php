...
$akun_view = NULL; // Initialize page object first

class cakun_view extends cakun {
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					...
					if (!$bMatchRecord) {
						if ($this->getSuccessMessage() == "" && $this->getFailureMessage() == "")
							$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record message
						$sReturnUrl = "akunlist.php"; // No matching record, return to list
					} else {
						$this->LoadRowValues($this->Recordset); // Load row values
					}
			}
		} else {
			$sReturnUrl = "akunlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($akun_view)) $akun_view = new cakun_view();
...
var CurrentForm = fakunview = new ew_Form("fakunview", "view");

// Form_CustomValidate event
fakunview.Form_CustomValidate = 
...
fakunview.ValidateRequired = true;
<?php } else { ?>
fakunview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fakunview.Lists["x_group_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_name","","",""],"ParentFields":[],"ChildFields":["x_subgrup_id"],"FilterFields":[],"Options":[],"Template":"","LinkTable":"grup"};
fakunview.Lists["x_subgrup_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_kode","x_nama","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"subgrup"};
fakunview.Lists["x_matauang_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_nama","x_kode","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"matauang"};
...
<div class="ewToolbar">
<?php if (!$akun_view->IsModal) { ?>
<?php $Breadcrumb->Render(); ?>
<?php } ?>
<?php $akun_view->ExportOptions->Render("body") ?>
<?php
	foreach ($akun_view->OtherOptions as &$option)
		$option->Render("body");
?>
<?php if (!$akun_view->IsModal) { ?>
...
<?php $akun_view->ShowPageHeader(); ?>
<?php
$akun_view->ShowMessage();
?>
<form name="fakunview" id="fakunview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($akun_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $akun_view->Token ?>">
<?php } ?>
...
<?php if ($akun->group_id->Visible) { // group_id ?>
	<tr id="r_group_id">
		<td><span id="elh_akun_group_id"><?php echo $akun->group_id->FldCaption() ?></span></td>
...
<?php echo $akun->group_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($akun->subgrup_id->Visible) { // subgrup_id ?>
	<tr id="r_subgrup_id">
		<td><span id="elh_akun_subgrup_id"><?php echo $akun->subgrup_id->FldCaption() ?></span></td>
...
<?php echo $akun->subgrup_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if ($akun->matauang_id->Visible) { // matauang_id ?>
	<tr id="r_matauang_id">
		<td><span id="elh_akun_matauang_id"><?php echo $akun->matauang_id->FldCaption() ?></span></td>
...
<?php echo $akun->matauang_id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
...
<?php if (!$akun_view->IsModal) { ?>
<?php if (!isset($akun_view->Pager)) $akun_view->Pager = new cPrevNextPager($akun_view->StartRec, $akun_view->DisplayRecs, $akun_view->TotalRecs) ?>
<?php if ($akun_view->Pager->RecordCount > 0 && $akun_view->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($akun_view->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $akun_view->PageUrl() ?>start=<?php echo $akun_view->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($akun_view->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $akun_view->PageUrl() ?>start=<?php echo $akun_view->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $akun_view->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($akun_view->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $akun_view->PageUrl() ?>start=<?php echo $akun_view->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($akun_view->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $akun_view->PageUrl() ?>start=<?php echo $akun_view->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $akun_view->Pager->PageCount ?></span>
</div>
<?php } ?>
<div class="clearfix"></div>
<?php } ?>
