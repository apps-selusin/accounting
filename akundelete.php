...
$akun_delete = NULL; // Initialize page object first

class cakun_delete extends cakun {
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("akunlist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in akun class, akuninfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("akunlist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($akun_delete)) $akun_delete = new cakun_delete();

// Page init
$akun_delete->Page_Init();

// Page main
$akun_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$akun_delete->Page_Render();
?>
...
var CurrentForm = fakundelete = new ew_Form("fakundelete", "delete");

// Form_CustomValidate event
fakundelete.Form_CustomValidate = 
...
fakundelete.ValidateRequired = true;
<?php } else { ?>
fakundelete.ValidateRequired = false; 
<?php } ?>
...
<?php $akun_delete->ShowPageHeader(); ?>
<?php
$akun_delete->ShowMessage();
?>
<form name="fakundelete" id="fakundelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($akun_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $akun_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="akun">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($akun_delete->RecKeys as $key) { ?>
...
<?php } ?>
	...
	<tbody>
<?php
$akun_delete->RecCnt = 0;
$i = 0;
while (!$akun_delete->Recordset->EOF) {
	$akun_delete->RecCnt++;
	$akun_delete->RowCnt++;

	// Set row properties
	$akun->ResetAttrs();
	$akun->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$akun_delete->LoadRowValues($akun_delete->Recordset);
...
<?php } ?>
<?php if ($akun->kode->Visible) { // kode ?>
		<td<?php echo $akun->kode->CellAttributes() ?>>
<span id="el<?php echo $akun_delete->RowCnt ?>_akun_kode" class="akun_kode">
...
<?php } ?>
<?php if ($akun->nama->Visible) { // nama ?>
		<td<?php echo $akun->nama->CellAttributes() ?>>
...
<?php } ?>
<?php if ($akun->matauang_id->Visible) { // matauang_id ?>
		<td<?php echo $akun->matauang_id->CellAttributes() ?>>
<span id="el<?php echo $akun_delete->RowCnt ?>_akun_matauang_id" class="akun_matauang_id">
...
<?php
	$akun_delete->Recordset->MoveNext();
}
$akun_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $akun_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fakundelete.Init();
