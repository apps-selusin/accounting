...
$subgrup_list = NULL; // Initialize page object first

class csubgrup_list extends csubgrup {
	...
	// Table name
	var $TableName = 'subgrup';

	// Page object name
	var $PageObjName = 'subgrup_list';

	// Grid form hidden field names
	var $FormName = 'fsubgruplist';
	...
	function __construct() {
		...
		// Table object (subgrup)
		if (!isset($GLOBALS["subgrup"]) || get_class($GLOBALS["subgrup"]) == "csubgrup") {
			$GLOBALS["subgrup"] = &$this;
		...
		if (!defined("EW_TABLE_NAME"))
			define("EW_TABLE_NAME", 'subgrup', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fsubgruplistsrch";
	...
	function Page_Terminate($url = "") {
		...
		// Export
		global $EW_EXPORT, $subgrup;
		if ($this->CustomExport <> "" && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, $EW_EXPORT)) {
				$sContent = ob_get_contents();
			if ($gsExportFile == "") $gsExportFile = $this->TableVar;
			$class = $EW_EXPORT[$this->CustomExport];
			if (class_exists($class)) {
				$doc = new $class($subgrup);
				$doc->Text = $sContent;
	...
	function GetFilterList() {
		...
		if (EW_SEARCH_FILTER_OPTION == "Server") {
			$sSavedFilterList = isset($UserProfile) ? $UserProfile->GetSearchFilters(CurrentUserName(), "fsubgruplistsrch") : "";
		} else {
			$sSavedFilterList = "";
		}
	...
	function ProcessFilterList() {
		...
		if (@$_POST["ajax"] == "savefilters") { // Save filter request (Ajax)
			...
			$UserProfile->SetSearchFilters(CurrentUserName(), "fsubgruplistsrch", $filters);
	...
	function RestoreFilterList() {
		...
		// Field grup_id
		$this->grup_id->AdvancedSearch->SearchValue = @$filter["x_grup_id"];
		$this->grup_id->AdvancedSearch->SearchOperator = @$filter["z_grup_id"];
		$this->grup_id->AdvancedSearch->SearchCondition = @$filter["v_grup_id"];
		$this->grup_id->AdvancedSearch->SearchValue2 = @$filter["y_grup_id"];
		$this->grup_id->AdvancedSearch->SearchOperator2 = @$filter["w_grup_id"];
		$this->grup_id->AdvancedSearch->Save();
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fsubgruplistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fsubgruplistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fsubgruplist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
	...
	function SetupSearchOptions() {
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $Language->Phrase("SearchPanel") . "\" data-caption=\"" . $Language->Phrase("SearchPanel") . "\" data-toggle=\"button\" data-form=\"fsubgruplistsrch\">" . $Language->Phrase("SearchBtn") . "</button>";
		$item->Visible = TRUE;
	...
	function LoadRowValues(&$rs) {
		...
		$this->grup_id->setDbValue($rs->fields('grup_id'));
	...
	function LoadDbValues(&$rs) {
		...
		$this->grup_id->DbValue = $row['grup_id'];
...
// Create page object
if (!isset($subgrup_list)) $subgrup_list = new csubgrup_list();

// Page init
$subgrup_list->Page_Init();

// Page main
$subgrup_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$subgrup_list->Page_Render();
?>
...
var CurrentForm = fsubgruplist = new ew_Form("fsubgruplist", "list");
fsubgruplist.FormKeyCountName = '<?php echo $subgrup_list->FormKeyCountName ?>';

// Form_CustomValidate event
fsubgruplist.Form_CustomValidate = 
...
fsubgruplist.ValidateRequired = true;
<?php } else { ?>
fsubgruplist.ValidateRequired = false; 
<?php } ?>
...
<?php if ($subgrup_list->TotalRecs > 0 && $subgrup_list->ExportOptions->Visible()) { ?>
<?php $subgrup_list->ExportOptions->Render("body") ?>
<?php } ?>
<?php if ($subgrup_list->SearchOptions->Visible()) { ?>
<?php $subgrup_list->SearchOptions->Render("body") ?>
<?php } ?>
<?php if ($subgrup_list->FilterOptions->Visible()) { ?>
<?php $subgrup_list->FilterOptions->Render("body") ?>
<?php } ?>
...
<?php
	...
	if ($bSelectLimit) {
		if ($subgrup_list->TotalRecs <= 0)
			$subgrup_list->TotalRecs = $subgrup->SelectRecordCount();
	} else {
		if (!$subgrup_list->Recordset && ($subgrup_list->Recordset = $subgrup_list->LoadRecordset()))
			$subgrup_list->TotalRecs = $subgrup_list->Recordset->RecordCount();
	}
	$subgrup_list->StartRec = 1;
	if ($subgrup_list->DisplayRecs <= 0 || ($subgrup->Export <> "" && $subgrup->ExportAll)) // Display all records
		$subgrup_list->DisplayRecs = $subgrup_list->TotalRecs;
	if (!($subgrup->Export <> "" && $subgrup->ExportAll))
		$subgrup_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$subgrup_list->Recordset = $subgrup_list->LoadRecordset($subgrup_list->StartRec-1, $subgrup_list->DisplayRecs);

	// Set no record found message
	if ($subgrup->CurrentAction == "" && $subgrup_list->TotalRecs == 0) {
		if ($subgrup_list->SearchWhere == "0=101")
			$subgrup_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$subgrup_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$subgrup_list->RenderOtherOptions();
?>
<?php if ($subgrup->Export == "" && $subgrup->CurrentAction == "") { ?>
<form name="fsubgruplistsrch" id="fsubgruplistsrch" class="form-inline ewForm" action="<?php echo ew_CurrentPage() ?>">
<?php $SearchPanelClass = ($subgrup_list->SearchWhere <> "") ? " in" : " in"; ?>
<div id="fsubgruplistsrch_SearchPanel" class="ewSearchPanel collapse<?php echo $SearchPanelClass ?>">
...
<?php } ?>
<?php $subgrup_list->ShowPageHeader(); ?>
<?php
$subgrup_list->ShowMessage();
?>
<?php if ($subgrup_list->TotalRecs > 0 || $subgrup->CurrentAction <> "") { ?>
<div class="panel panel-default ewGrid subgrup">
<form name="fsubgruplist" id="fsubgruplist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($subgrup_list->CheckToken) { ?>
...
// Header row
$subgrup_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$subgrup_list->RenderListOptions();
...
$subgrup_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($subgrup->ExportAll && $subgrup->Export <> "") {
	$subgrup_list->StopRec = $subgrup_list->TotalRecs;
} else {

	// Set the last record to display
	if ($subgrup_list->TotalRecs > $subgrup_list->StartRec + $subgrup_list->DisplayRecs - 1)
		$subgrup_list->StopRec = $subgrup_list->StartRec + $subgrup_list->DisplayRecs - 1;
	else
		$subgrup_list->StopRec = $subgrup_list->TotalRecs;
}
$subgrup_list->RecCnt = $subgrup_list->StartRec - 1;
if ($subgrup_list->Recordset && !$subgrup_list->Recordset->EOF) {
	$subgrup_list->Recordset->MoveFirst();
	$bSelectLimit = $subgrup_list->UseSelectLimit;
	if (!$bSelectLimit && $subgrup_list->StartRec > 1)
		$subgrup_list->Recordset->Move($subgrup_list->StartRec - 1);
} elseif (!$subgrup->AllowAddDeleteRow && $subgrup_list->StopRec == 0) {
	$subgrup_list->StopRec = $subgrup->GridAddRowCount;
}
...
$subgrup->ResetAttrs();
$subgrup_list->RenderRow();
while ($subgrup_list->RecCnt < $subgrup_list->StopRec) {
	$subgrup_list->RecCnt++;
	if (intval($subgrup_list->RecCnt) >= intval($subgrup_list->StartRec)) {
		$subgrup_list->RowCnt++;

		// Set up key count
		$subgrup_list->KeyCount = $subgrup_list->RowIndex;
		...
		if ($subgrup->CurrentAction == "gridadd") {
			...
			$subgrup_list->LoadRowValues($subgrup_list->Recordset); // Load row values
		}
		$subgrup->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$subgrup->RowAttrs = array_merge($subgrup->RowAttrs, array('data-rowindex'=>$subgrup_list->RowCnt, 'id'=>'r' . $subgrup_list->RowCnt . '_subgrup', 'data-rowtype'=>$subgrup->RowType));
...
<span id="el<?php echo $subgrup_list->RowCnt ?>_subgrup_kode" class="subgrup_kode">
...
<span id="el<?php echo $subgrup_list->RowCnt ?>_subgrup_nama" class="subgrup_nama">
...
$subgrup_list->ListOptions->Render("body", "right", $subgrup_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($subgrup->CurrentAction <> "gridadd")
		$subgrup_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($subgrup->CurrentAction == "") { ?>
...
// Close recordset
if ($subgrup_list->Recordset)
	$subgrup_list->Recordset->Close();
?>
<div class="panel-footer ewGridLowerPanel">
<?php if ($subgrup->CurrentAction <> "gridadd" && $subgrup->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-inline ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($subgrup_list->Pager)) $subgrup_list->Pager = new cPrevNextPager($subgrup_list->StartRec, $subgrup_list->DisplayRecs, $subgrup_list->TotalRecs) ?>
<?php if ($subgrup_list->Pager->RecordCount > 0 && $subgrup_list->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($subgrup_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $subgrup_list->PageUrl() ?>start=<?php echo $subgrup_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($subgrup_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $subgrup_list->PageUrl() ?>start=<?php echo $subgrup_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $subgrup_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($subgrup_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $subgrup_list->PageUrl() ?>start=<?php echo $subgrup_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($subgrup_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $subgrup_list->PageUrl() ?>start=<?php echo $subgrup_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $subgrup_list->Pager->PageCount ?></span>
</div>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $subgrup_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $subgrup_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $subgrup_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
</form>
<?php } ?>
...
<?php
	foreach ($subgrup_list->OtherOptions as &$option)
...
<?php } ?>
<?php if ($subgrup_list->TotalRecs == 0 && $subgrup->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($subgrup_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
...
<?php } ?>
<script type="text/javascript">
fsubgruplistsrch.FilterList = <?php echo $subgrup_list->GetFilterList() ?>;
fsubgruplistsrch.Init();
fsubgruplist.Init();
</script>
<?php
$subgrup_list->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
