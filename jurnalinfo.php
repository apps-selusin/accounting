...
class cjurnal extends cTable {
	...
	function __construct() {
		...
		// id
		$this->id = new cField('jurnal', 'jurnal', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`jurnal`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("jurnalview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("jurnalview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "jurnaladd.php?" . $this->UrlParm($parm);
		else
			$url = "jurnaladd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("jurnaldelete.php", $this->UrlParm());
	}
