...
$jurnal_add = NULL; // Initialize page object first

class cjurnal_add extends cjurnal {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "jurnalview.php")
	...
	function LoadDefaultValues() {
		...
		$this->period_id->CurrentValue = NULL;
		$this->period_id->OldValue = $this->period_id->CurrentValue;
		$this->createon->CurrentValue = NULL;
		$this->createon->OldValue = $this->createon->CurrentValue;
		$this->keterangan->CurrentValue = NULL;
		$this->keterangan->OldValue = $this->keterangan->CurrentValue;
		$this->person_id->CurrentValue = NULL;
		$this->person_id->OldValue = $this->person_id->CurrentValue;
	...
	function ValidateForm() {
		...
		if (in_array("jurnald", $DetailTblVar) && $GLOBALS["jurnald"]->DetailAdd) {
	...
	function AddRow($rsold = NULL) {
		...
		// period_id
		$this->period_id->SetDbValueDef($rsnew, $this->period_id->CurrentValue, NULL, FALSE);

		// createon
		$this->createon->SetDbValueDef($rsnew, ew_UnFormatDateTime($this->createon->CurrentValue, 0), NULL, FALSE);

		// keterangan
		$this->keterangan->SetDbValueDef($rsnew, $this->keterangan->CurrentValue, NULL, FALSE);

		// person_id
		$this->person_id->SetDbValueDef($rsnew, $this->person_id->CurrentValue, NULL, FALSE);
		...
		if ($AddRow) {
			$DetailTblVar = explode(",", $this->getCurrentDetailTable());
			if (in_array("jurnald", $DetailTblVar) && $GLOBALS["jurnald"]->DetailAdd) {
				$GLOBALS["jurnald"]->jurnal_id->setSessionValue($this->id->CurrentValue); // Set master key
				if (!isset($GLOBALS["jurnald_grid"])) $GLOBALS["jurnald_grid"] = new cjurnald_grid(); // Get detail page object
				$AddRow = $GLOBALS["jurnald_grid"]->GridInsert();
				if (!$AddRow)
					$GLOBALS["jurnald"]->jurnal_id->setSessionValue(""); // Clear master key if insert failed
			}
		}
...
// Create page object
if (!isset($jurnal_add)) $jurnal_add = new cjurnal_add();

// Page init
$jurnal_add->Page_Init();

// Page main
$jurnal_add->Page_Main();
...
var CurrentForm = fjurnaladd = new ew_Form("fjurnaladd", "add");
...
fjurnaladd.Form_CustomValidate = 
...
fjurnaladd.ValidateRequired = true;
<?php } else { ?>
fjurnaladd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fjurnaladd.Lists["x_tipejurnal_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_nama","","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"tipejurnal"};
fjurnaladd.Lists["x_period_id"] = {"LinkField":"x_id","Ajax":true,"AutoFill":false,"DisplayFields":["x_start","x_end","",""],"ParentFields":[],"ChildFields":[],"FilterFields":[],"Options":[],"Template":"","LinkTable":"periode"};
...
?>
<form name="fjurnaladd" id="fjurnaladd" class="<?php echo $jurnal_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($jurnal_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $jurnal_add->Token ?>">
<?php } ?>
...
<?php
	if (in_array("jurnald", explode(",", $jurnal->getCurrentDetailTable())) && $jurnald->DetailAdd) {
?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $jurnal_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
