...
$subgrup_add = NULL; // Initialize page object first

class csubgrup_add extends csubgrup {
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("subgruplist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "subgrupview.php")
	...
	function AddRow($rsold = NULL) {
		...
		// grup_id
		$this->grup_id->SetDbValueDef($rsnew, $this->grup_id->CurrentValue, NULL, FALSE);
...
// Create page object
if (!isset($subgrup_add)) $subgrup_add = new csubgrup_add();

// Page init
$subgrup_add->Page_Init();

// Page main
$subgrup_add->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$subgrup_add->Page_Render();
?>
...
var CurrentForm = fsubgrupadd = new ew_Form("fsubgrupadd", "add");

// Validate form
fsubgrupadd.Validate = function() {
...
fsubgrupadd.Form_CustomValidate = 
...
fsubgrupadd.ValidateRequired = true;
<?php } else { ?>
fsubgrupadd.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $subgrup_add->ShowPageHeader(); ?>
<?php
$subgrup_add->ShowMessage();
?>
<form name="fsubgrupadd" id="fsubgrupadd" class="<?php echo $subgrup_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($subgrup_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $subgrup_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $subgrup_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
