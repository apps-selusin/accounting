...
//
// Table class for pengiriman
//
class cpengiriman extends cTable {
	var $id;
	var $kode;
	var $nama;
	var $akunjual;
	...
	function __construct() {
		...
		$this->TableVar = 'pengiriman';
		$this->TableName = 'pengiriman';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`pengiriman`";
		$this->DBID = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new cField('pengiriman', 'pengiriman', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = TRUE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// kode
		$this->kode = new cField('pengiriman', 'pengiriman', 'x_kode', 'kode', '`kode`', '`kode`', 200, -1, FALSE, '`kode`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->kode->Sortable = TRUE; // Allow sort
		$this->fields['kode'] = &$this->kode;

		// nama
		$this->nama = new cField('pengiriman', 'pengiriman', 'x_nama', 'nama', '`nama`', '`nama`', 200, -1, FALSE, '`nama`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->nama->Sortable = TRUE; // Allow sort
		$this->fields['nama'] = &$this->nama;

		// akunjual
		$this->akunjual = new cField('pengiriman', 'pengiriman', 'x_akunjual', 'akunjual', '`akunjual`', '`akunjual`', 3, -1, FALSE, '`akunjual`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// akunbeli
		$this->akunbeli = new cField('pengiriman', 'pengiriman', 'x_akunbeli', 'akunbeli', '`akunbeli`', '`akunbeli`', 3, -1, FALSE, '`akunbeli`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->akunbeli->Sortable = TRUE; // Allow sort
		$this->akunbeli->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['akunbeli'] = &$this->akunbeli;

		// keterangan
		$this->keterangan = new cField('pengiriman', 'pengiriman', 'x_keterangan', 'keterangan', '`keterangan`', '`keterangan`', 200, -1, FALSE, '`keterangan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->keterangan->Sortable = TRUE; // Allow sort
		$this->fields['keterangan'] = &$this->keterangan;

		// tipe
		$this->tipe = new cField('pengiriman', 'pengiriman', 'x_tipe', 'tipe', '`tipe`', '`tipe`', 3, -1, FALSE, '`tipe`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`pengiriman`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("pengirimanview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("pengirimanview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "pengirimanadd.php?" . $this->UrlParm($parm);
		else
			$url = "pengirimanadd.php";
		return $this->AddMasterUrl($url);
	}

	// Edit URL
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("pengirimanedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("pengirimanadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("pengirimandelete.php", $this->UrlParm());
	}
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->akunbeli->Exportable) $Doc->ExportCaption($this->akunbeli);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->akunbeli->Exportable) $Doc->ExportField($this->akunbeli);
