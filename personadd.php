...
$person_add = NULL; // Initialize page object first

class cperson_add extends cperson {
		...
		switch ($this->CurrentAction) {
			...
			case "C": // Copy an existing record
				if (!$this->LoadRow()) { // Load record based on key
					...
					$this->Page_Terminate("personlist.php"); // No matching record, return to list
				}
				break;
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					elseif (ew_GetPageName($sReturnUrl) == "personview.php")
	...
	function LoadDefaultValues() {
		...
		$this->telp1->CurrentValue = NULL;
		$this->telp1->OldValue = $this->telp1->CurrentValue;
		...
		$this->telp2->CurrentValue = NULL;
		$this->telp2->OldValue = $this->telp2->CurrentValue;
		...
		$this->klasifikasi_id->CurrentValue = NULL;
		$this->klasifikasi_id->OldValue = $this->klasifikasi_id->CurrentValue;
	...
	function AddRow($rsold = NULL) {
		...
		// telp1
		$this->telp1->SetDbValueDef($rsnew, $this->telp1->CurrentValue, NULL, FALSE);
		...
		// telp2
		$this->telp2->SetDbValueDef($rsnew, $this->telp2->CurrentValue, NULL, FALSE);
		...
		// klasifikasi_id
		$this->klasifikasi_id->SetDbValueDef($rsnew, $this->klasifikasi_id->CurrentValue, NULL, FALSE);
...
// Create page object
if (!isset($person_add)) $person_add = new cperson_add();
...
var CurrentForm = fpersonadd = new ew_Form("fpersonadd", "add");
...
fpersonadd.ValidateRequired = true;
<?php } else { ?>
fpersonadd.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $person_add->ShowPageHeader(); ?>
<?php
$person_add->ShowMessage();
?>
<form name="fpersonadd" id="fpersonadd" class="<?php echo $person_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($person_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $person_add->Token ?>">
<?php } ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $person_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
