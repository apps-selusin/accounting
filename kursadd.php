...
$kurs_add = NULL; // Initialize page object first

class ckurs_add extends ckurs {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "kurslist.php")
						$sReturnUrl = $this->AddMasterUrl($sReturnUrl); // List page, return to list page with correct master key if necessary
					elseif (ew_GetPageName($sReturnUrl) == "kursview.php")
	...
	function RenderRow() {
		...
		$this->nilai->ViewCustomAttributes = "";
			...
			$this->tanggal->EditValue = ew_HtmlEncode($this->tanggal->CurrentValue);
	...
	function AddRow($rsold = NULL) {
		...
		// tanggal
		$this->tanggal->SetDbValueDef($rsnew, $this->tanggal->CurrentValue, NULL, FALSE);
...
// Create page object
if (!isset($kurs_add)) $kurs_add = new ckurs_add();
...
var CurrentForm = fkursadd = new ew_Form("fkursadd", "add");

// Validate form
fkursadd.Validate = function() {
	...
	for (var i = startcnt; i <= rowcnt; i++) {
		...
		$fobj.data("rowindex", infix);
			...
			if (elm && !ew_CheckInteger(elm.value))
				return this.OnError(elm, "<?php echo ew_JsEncode2($kurs->nilai->FldErrMsg()) ?>");
...
fkursadd.ValidateRequired = true;
<?php } else { ?>
fkursadd.ValidateRequired = false; 
<?php } ?>
...
<?php } ?>
<?php $kurs_add->ShowPageHeader(); ?>
<?php
$kurs_add->ShowMessage();
?>
<form name="fkursadd" id="fkursadd" class="<?php echo $kurs_add->FormClassName ?>" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($kurs_add->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $kurs_add->Token ?>">
<?php } ?>
...
<?php if ($kurs->tanggal->Visible) { // tanggal ?>
	<div id="r_tanggal" class="form-group">
		<label id="elh_kurs_tanggal" for="x_tanggal" class="col-sm-2 control-label ewLabel"><?php echo $kurs->tanggal->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $kurs->tanggal->CellAttributes() ?>>
<span id="el_kurs_tanggal">
<input type="text" data-table="kurs" data-field="x_tanggal" name="x_tanggal" id="x_tanggal" size="30" placeholder="<?php echo ew_HtmlEncode($kurs->tanggal->getPlaceHolder()) ?>" value="<?php echo $kurs->tanggal->EditValue ?>"<?php echo $kurs->tanggal->EditAttributes() ?>>
</span>
<?php echo $kurs->tanggal->CustomMsg ?></div></div>
	</div>
<?php } ?>
<?php if ($kurs->nilai->Visible) { // nilai ?>
	<div id="r_nilai" class="form-group">
		<label id="elh_kurs_nilai" for="x_nilai" class="col-sm-2 control-label ewLabel"><?php echo $kurs->nilai->FldCaption() ?></label>
		<div class="col-sm-10"><div<?php echo $kurs->nilai->CellAttributes() ?>>
<span id="el_kurs_nilai">
<input type="text" data-table="kurs" data-field="x_nilai" name="x_nilai" id="x_nilai" size="30" placeholder="<?php echo ew_HtmlEncode($kurs->nilai->getPlaceHolder()) ?>" value="<?php echo $kurs->nilai->EditValue ?>"<?php echo $kurs->nilai->EditAttributes() ?>>
</span>
<?php echo $kurs->nilai->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div>
<?php if (!$kurs_add->IsModal) { ?>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $kurs_add->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
	</div>
</div>
<?php } ?>
</form>
