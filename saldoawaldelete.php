...
$saldoawal_delete = NULL; // Initialize page object first

class csaldoawal_delete extends csaldoawal {
	...
	// Page object name
	var $PageObjName = 'saldoawal_delete';
	...
	function Page_Main() {
		...
		if ($sFilter == "")
			$this->Page_Terminate("saldoawallist.php"); // Prevent SQL injection, return to list

		// Set up filter (SQL WHHERE clause) and get return SQL
		// SQL constructor in saldoawal class, saldoawalinfo.php
		...
		if ($this->CurrentAction == "I") { // Load records for display
			...
			if ($this->TotalRecs <= 0) { // No record found, exit
				...
				$this->Page_Terminate("saldoawallist.php"); // Return to list
			}
		}
	}
...
// Create page object
if (!isset($saldoawal_delete)) $saldoawal_delete = new csaldoawal_delete();

// Page init
$saldoawal_delete->Page_Init();

// Page main
$saldoawal_delete->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$saldoawal_delete->Page_Render();
?>
...
var CurrentForm = fsaldoawaldelete = new ew_Form("fsaldoawaldelete", "delete");

// Form_CustomValidate event
fsaldoawaldelete.Form_CustomValidate = 
...
fsaldoawaldelete.ValidateRequired = true;
<?php } else { ?>
fsaldoawaldelete.ValidateRequired = false; 
<?php } ?>
...
<?php $saldoawal_delete->ShowPageHeader(); ?>
<?php
$saldoawal_delete->ShowMessage();
?>
<form name="fsaldoawaldelete" id="fsaldoawaldelete" class="form-inline ewForm ewDeleteForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($saldoawal_delete->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $saldoawal_delete->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="saldoawal">
<input type="hidden" name="a_delete" id="a_delete" value="D">
<?php foreach ($saldoawal_delete->RecKeys as $key) { ?>
...
<?php if ($saldoawal->debet->Visible) { // debet ?>
		<th><span id="elh_saldoawal_debet" class="saldoawal_debet"><?php echo $saldoawal->debet->FldCaption() ?></span></th>
<?php } ?>
...
<?php } ?>
	...
	<tbody>
<?php
$saldoawal_delete->RecCnt = 0;
$i = 0;
while (!$saldoawal_delete->Recordset->EOF) {
	$saldoawal_delete->RecCnt++;
	$saldoawal_delete->RowCnt++;

	// Set row properties
	$saldoawal->ResetAttrs();
	$saldoawal->RowType = EW_ROWTYPE_VIEW; // View

	// Get the field contents
	$saldoawal_delete->LoadRowValues($saldoawal_delete->Recordset);
...
<?php } ?>
<?php if ($saldoawal->debet->Visible) { // debet ?>
		<td<?php echo $saldoawal->debet->CellAttributes() ?>>
<span id="el<?php echo $saldoawal_delete->RowCnt ?>_saldoawal_debet" class="saldoawal_debet">
...
<?php } ?>
<?php if ($saldoawal->kredit->Visible) { // kredit ?>
		<td<?php echo $saldoawal->kredit->CellAttributes() ?>>
...
<?php } ?>
<?php if ($saldoawal->saldo->Visible) { // saldo ?>
		<td<?php echo $saldoawal->saldo->CellAttributes() ?>>
<span id="el<?php echo $saldoawal_delete->RowCnt ?>_saldoawal_saldo" class="saldoawal_saldo">
...
<?php
	$saldoawal_delete->Recordset->MoveNext();
}
$saldoawal_delete->Recordset->Close();
?>
</tbody>
</table>
</div>
...
<button class="btn btn-default ewButton" name="btnCancel" id="btnCancel" type="button" data-href="<?php echo $saldoawal_delete->getReturnUrl() ?>"><?php echo $Language->Phrase("CancelBtn") ?></button>
</div>
</form>
<script type="text/javascript">
fsaldoawaldelete.Init();
</script>
<?php
$saldoawal_delete->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$saldoawal_delete->Page_Terminate();
?>
