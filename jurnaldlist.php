...
$jurnald_list = NULL; // Initialize page object first

class cjurnald_list extends cjurnald {
	...
	// Page object name
	var $PageObjName = 'jurnald_list';

	// Grid form hidden field names
	var $FormName = 'fjurnaldlist';
	...
	function __construct() {
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fjurnaldlistsrch";
	...
	function SetupOtherOptions() {
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fjurnaldlistsrch\" href=\"#\">" . $Language->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = FALSE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fjurnaldlistsrch\" href=\"#\">" . $Language->Phrase("DeleteFilter") . "</a>";
		$item->Visible = FALSE;
		...
		$options = &$this->OtherOptions;
			...
			foreach ($this->ListActions->Items as $listaction) {
				if ($listaction->Select == EW_ACTION_MULTIPLE) {
					...
					$item->Body = "<a class=\"ewAction ewListAction\" title=\"" . ew_HtmlEncode($caption) . "\" data-caption=\"" . ew_HtmlEncode($caption) . "\" href=\"\" onclick=\"ew_SubmitAction(event,jQuery.extend({f:document.fjurnaldlist}," . $listaction->ToJson(TRUE) . "));return false;\">" . $icon . "</a>";
...
// Create page object
if (!isset($jurnald_list)) $jurnald_list = new cjurnald_list();

// Page init
$jurnald_list->Page_Init();

// Page main
$jurnald_list->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$jurnald_list->Page_Render();
?>
...
var CurrentForm = fjurnaldlist = new ew_Form("fjurnaldlist", "list");
fjurnaldlist.FormKeyCountName = '<?php echo $jurnald_list->FormKeyCountName ?>';

// Form_CustomValidate event
fjurnaldlist.Form_CustomValidate = 
...
fjurnaldlist.ValidateRequired = true;
<?php } else { ?>
fjurnaldlist.ValidateRequired = false; 
<?php } ?>
...
<?php if ($jurnald_list->TotalRecs > 0 && $jurnald_list->ExportOptions->Visible()) { ?>
<?php $jurnald_list->ExportOptions->Render("body") ?>
<?php } ?>
...
<?php
	...
	if ($bSelectLimit) {
		if ($jurnald_list->TotalRecs <= 0)
			$jurnald_list->TotalRecs = $jurnald->SelectRecordCount();
	} else {
		if (!$jurnald_list->Recordset && ($jurnald_list->Recordset = $jurnald_list->LoadRecordset()))
			$jurnald_list->TotalRecs = $jurnald_list->Recordset->RecordCount();
	}
	$jurnald_list->StartRec = 1;
	if ($jurnald_list->DisplayRecs <= 0 || ($jurnald->Export <> "" && $jurnald->ExportAll)) // Display all records
		$jurnald_list->DisplayRecs = $jurnald_list->TotalRecs;
	if (!($jurnald->Export <> "" && $jurnald->ExportAll))
		$jurnald_list->SetUpStartRec(); // Set up start record position
	if ($bSelectLimit)
		$jurnald_list->Recordset = $jurnald_list->LoadRecordset($jurnald_list->StartRec-1, $jurnald_list->DisplayRecs);

	// Set no record found message
	if ($jurnald->CurrentAction == "" && $jurnald_list->TotalRecs == 0) {
		if ($jurnald_list->SearchWhere == "0=101")
			$jurnald_list->setWarningMessage($Language->Phrase("EnterSearchCriteria"));
		else
			$jurnald_list->setWarningMessage($Language->Phrase("NoRecord"));
	}
$jurnald_list->RenderOtherOptions();
?>
<?php $jurnald_list->ShowPageHeader(); ?>
<?php
$jurnald_list->ShowMessage();
?>
<?php if ($jurnald_list->TotalRecs > 0 || $jurnald->CurrentAction <> "") { ?>
<div class="panel panel-default ewGrid jurnald">
<form name="fjurnaldlist" id="fjurnaldlist" class="form-inline ewForm ewListForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($jurnald_list->CheckToken) { ?>
...
// Header row
$jurnald_list->RowType = EW_ROWTYPE_HEADER;

// Render list options
$jurnald_list->RenderListOptions();
...
$jurnald_list->ListOptions->Render("header", "right");
?>
	</tr>
</thead>
<tbody>
<?php
if ($jurnald->ExportAll && $jurnald->Export <> "") {
	$jurnald_list->StopRec = $jurnald_list->TotalRecs;
} else {

	// Set the last record to display
	if ($jurnald_list->TotalRecs > $jurnald_list->StartRec + $jurnald_list->DisplayRecs - 1)
		$jurnald_list->StopRec = $jurnald_list->StartRec + $jurnald_list->DisplayRecs - 1;
	else
		$jurnald_list->StopRec = $jurnald_list->TotalRecs;
}
$jurnald_list->RecCnt = $jurnald_list->StartRec - 1;
if ($jurnald_list->Recordset && !$jurnald_list->Recordset->EOF) {
	$jurnald_list->Recordset->MoveFirst();
	$bSelectLimit = $jurnald_list->UseSelectLimit;
	if (!$bSelectLimit && $jurnald_list->StartRec > 1)
		$jurnald_list->Recordset->Move($jurnald_list->StartRec - 1);
} elseif (!$jurnald->AllowAddDeleteRow && $jurnald_list->StopRec == 0) {
	$jurnald_list->StopRec = $jurnald->GridAddRowCount;
}

// Initialize aggregate
$jurnald->RowType = EW_ROWTYPE_AGGREGATEINIT;
$jurnald->ResetAttrs();
$jurnald_list->RenderRow();
while ($jurnald_list->RecCnt < $jurnald_list->StopRec) {
	$jurnald_list->RecCnt++;
	if (intval($jurnald_list->RecCnt) >= intval($jurnald_list->StartRec)) {
		$jurnald_list->RowCnt++;

		// Set up key count
		$jurnald_list->KeyCount = $jurnald_list->RowIndex;
		...
		if ($jurnald->CurrentAction == "gridadd") {
			...
			$jurnald_list->LoadRowValues($jurnald_list->Recordset); // Load row values
		}
		$jurnald->RowType = EW_ROWTYPE_VIEW; // Render view

		// Set up row id / data-rowindex
		$jurnald->RowAttrs = array_merge($jurnald->RowAttrs, array('data-rowindex'=>$jurnald_list->RowCnt, 'id'=>'r' . $jurnald_list->RowCnt . '_jurnald', 'data-rowtype'=>$jurnald->RowType));
...
<span id="el<?php echo $jurnald_list->RowCnt ?>_jurnald_debet" class="jurnald_debet">
...
<span id="el<?php echo $jurnald_list->RowCnt ?>_jurnald_kredit" class="jurnald_kredit">
...
$jurnald_list->ListOptions->Render("body", "right", $jurnald_list->RowCnt);
?>
	</tr>
<?php
	}
	if ($jurnald->CurrentAction <> "gridadd")
		$jurnald_list->Recordset->MoveNext();
}
?>
</tbody>
</table>
<?php } ?>
<?php if ($jurnald->CurrentAction == "") { ?>
...
// Close recordset
if ($jurnald_list->Recordset)
	$jurnald_list->Recordset->Close();
?>
<div class="panel-footer ewGridLowerPanel">
<?php if ($jurnald->CurrentAction <> "gridadd" && $jurnald->CurrentAction <> "gridedit") { ?>
<form name="ewPagerForm" class="ewForm form-inline ewPagerForm" action="<?php echo ew_CurrentPage() ?>">
<?php if (!isset($jurnald_list->Pager)) $jurnald_list->Pager = new cPrevNextPager($jurnald_list->StartRec, $jurnald_list->DisplayRecs, $jurnald_list->TotalRecs) ?>
<?php if ($jurnald_list->Pager->RecordCount > 0 && $jurnald_list->Pager->Visible) { ?>
<div class="ewPager">
<span><?php echo $Language->Phrase("Page") ?>&nbsp;</span>
<div class="ewPrevNext"><div class="input-group">
<div class="input-group-btn">
<!--first page button-->
	<?php if ($jurnald_list->Pager->FirstButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerFirst") ?>" href="<?php echo $jurnald_list->PageUrl() ?>start=<?php echo $jurnald_list->Pager->FirstButton->Start ?>"><span class="icon-first ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerFirst") ?>"><span class="icon-first ewIcon"></span></a>
	<?php } ?>
<!--previous page button-->
	<?php if ($jurnald_list->Pager->PrevButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerPrevious") ?>" href="<?php echo $jurnald_list->PageUrl() ?>start=<?php echo $jurnald_list->Pager->PrevButton->Start ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerPrevious") ?>"><span class="icon-prev ewIcon"></span></a>
	<?php } ?>
</div>
<!--current page number-->
	<input class="form-control input-sm" type="text" name="<?php echo EW_TABLE_PAGE_NO ?>" value="<?php echo $jurnald_list->Pager->CurrentPage ?>">
<div class="input-group-btn">
<!--next page button-->
	<?php if ($jurnald_list->Pager->NextButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerNext") ?>" href="<?php echo $jurnald_list->PageUrl() ?>start=<?php echo $jurnald_list->Pager->NextButton->Start ?>"><span class="icon-next ewIcon"></span></a>
	<?php } else { ?>
	<a class="btn btn-default btn-sm disabled" title="<?php echo $Language->Phrase("PagerNext") ?>"><span class="icon-next ewIcon"></span></a>
	<?php } ?>
<!--last page button-->
	<?php if ($jurnald_list->Pager->LastButton->Enabled) { ?>
	<a class="btn btn-default btn-sm" title="<?php echo $Language->Phrase("PagerLast") ?>" href="<?php echo $jurnald_list->PageUrl() ?>start=<?php echo $jurnald_list->Pager->LastButton->Start ?>"><span class="icon-last ewIcon"></span></a>
	<?php } else { ?>
...
<span>&nbsp;<?php echo $Language->Phrase("of") ?>&nbsp;<?php echo $jurnald_list->Pager->PageCount ?></span>
</div>
<div class="ewPager ewRec">
	<span><?php echo $Language->Phrase("Record") ?>&nbsp;<?php echo $jurnald_list->Pager->FromIndex ?>&nbsp;<?php echo $Language->Phrase("To") ?>&nbsp;<?php echo $jurnald_list->Pager->ToIndex ?>&nbsp;<?php echo $Language->Phrase("Of") ?>&nbsp;<?php echo $jurnald_list->Pager->RecordCount ?></span>
</div>
<?php } ?>
</form>
<?php } ?>
...
<?php
	foreach ($jurnald_list->OtherOptions as &$option)
...
<?php } ?>
<?php if ($jurnald_list->TotalRecs == 0 && $jurnald->CurrentAction == "") { // Show other options ?>
<div class="ewListOtherOptions">
<?php
	foreach ($jurnald_list->OtherOptions as &$option) {
		$option->ButtonClass = "";
